...
Page({
  ...
  goSureOrder() {
    ...
    wx.navigateTo({
      url: '/pages/orderEatIn/orderEatIn?shopId=' + this.data.list.cyDiningTable.shopId + 
      '&openId=' + wx.getStorageSync('userInfo_').openId + '&type=eatIn' + '&orderName=' +
       this.data.list.cyDiningTable.areaName + '-' + this.data.list.cyDiningTable.deskNum,
    })
  },
...
//显示隐藏优惠券
  ...
  getCoupons: function (e) { //领取优惠券
    ...
    wx.request({
      url: getApp().data.pathApi + '/utils/receiveCoupon',//自己的服务接口地址
      method: 'post',
      header: {
  ...
  getyouhuiData: function (shopId){
    var userInfo = wx.getStorageSync('userInfo_')
    let that = this;
    var openId = userInfo.openId;

    wx.request({
      url: getApp().data.pathApi + '/utils/getCouponInShopByShopId',
      method: 'post',
      header: {
        'content-type': 'application/x-www-form-urlencoded;charset=UTF-8'
      },
  ...
  onLoad: function (options) {
    ...
    app.http.request('/utils/getNewCyDataByTableId',{'tableId': scene}).then(res => {
      //var _youhui = res.data;
      let allFoodList = [...res.data.data.cyFoodStatusVoList, ...res.data.data.cyFoodTypeV0];
      let shopId = res.data.data.cyDiningTable.shopId;
      if (shopId){
        that.getyouhuiData(shopId);
      }
      that.setData({
        list : res.data.data,
