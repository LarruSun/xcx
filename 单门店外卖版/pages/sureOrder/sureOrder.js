...
Page({
  ...
  getCartData: function () {   //获取购物车数据
    ...
    if (this.data.couponId != null && this.data.couponId != '') {
      if (this.data.couponAmount != null) {
      ...
      } else if (this.data.couponDiscount != null){
        obj.totprice = (obj.totprice * (this.data.couponDiscount * 0.1)).toFixed(2)
      } else {
        wx.showModal({
          title: '提示',
          content: '该优惠券未达到满减金额!',
  ...
  clipCoupons: function () {
    if (null != this.data.couponId && "" != this.data.couponId) {
      wx.navigateTo({
        url: '../sureOrder/clipCoupons/clipCoupons?shopId=' + this.data.shopId + '&couponId=' + this.data.couponId + '&yuanjia=' + this.data.yuanjia,
      })
    }else {
      wx.navigateTo({
