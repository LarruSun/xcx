...
Page({
  ...
  getData: function (param, ifAdd) {
    ...
    var that = this
    console.log(app.chanpinfenlei.default)
    that.listPage.pageSize = app.chanpinfenlei.default.pageSize
    that.listPage.curPage = app.chanpinfenlei.default.curPage
    that.listPage.totalSize = app.chanpinfenlei.default.totalSize
    let dataArr = that.data.productData

    if (ifAdd == 2) {
      dataArr = []
    }
    if (!app.chanpinfenlei.default.result || app.chanpinfenlei.default.result.length == 0) {
      that.setData({ productData: null })
    } else {
      if (dataArr == null) { dataArr = [] }
      dataArr = dataArr.concat(app.chanpinfenlei.default.result)
