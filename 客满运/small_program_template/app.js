...
/* 转换成str */
  ...
  linkEvent: function (linkUrl) {
    ...
    if (linkUrl.substr(0, 3) == 'tel') {
      wx.navigateTo({
        url: '/pages/custom_page_contact/index?tel=' + linkUrl.substring(4, linkUrl.length),
      })
    }
    ...
    else if (linkUrl.substr(0, 9) == 'news_list') {
      wx.navigateTo({
        url: '/pages/' + linkUrl.substr(0, 9) + '/index' + urlData.param,
      })
    }
